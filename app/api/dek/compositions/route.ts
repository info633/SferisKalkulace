...
import type { NZUProgram, MeasureKey } from '../../../lib/dek/types'

export async function GET(req: NextRequest) {
  const { searchParams } = new URL(req.url)
  const program = (searchParams.get('program')||'OPRAV_DUM') as NZUProgram
  const measure = searchParams.get('measure') as MeasureKey | null
  ...
  let list = (all as any).compositions as any[]
  ...
  if (!isNaN(targetU)) list = list.filter(c => typeof c.U_W_m2K === 'number' && c.U_W_m2K <= targetU)
